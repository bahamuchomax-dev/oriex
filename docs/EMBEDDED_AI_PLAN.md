...
## Phase 3C-Fix1 Load-Failure Diagnosis + Retry

PC Chrome real run: WebGPU / IndexedDB / secureContext OK, but the WebLLM model load failed with `Failed to execute 'add' on 'Cache': Cache.add() encountered a network error` after ~16 s. Generation was never reached. This is treated as a **model/runtime fetch or Cache Storage write failure**, not a device-performance problem.

This phase improves the hidden PoC's diagnosis and retry (no production UI, flags stay false):
- `webLlmEngineLoader.js` gains `classifyWebLlmLoadError(rawError, ctx)` + `suggestedActionForLoadError(type)`. Types: `cache-add-network-error`, `model-fetch-or-cache-failed`, `webgpu-unavailable`, `indexeddb-unavailable`, `secure-context-required`, `offline`, `load-timeout`, `unknown-load-error`. The observed error maps to `cache-add-network-error` → `retry-after-reload-or-clear-site-data`.
- `EmbeddedAiPocPanel.jsx` shows retry guidance on failure (check connection, reload + retry, optionally clear site data manually, nothing saved), a 再試行 button, and a 診断ログをコピー button. Model/runtime sources are shown with a note that they are fetch sources, not an input-send destination.
- The diagnostic log adds `loadErrorType`, `loadErrorMessage`, `loadAttemptCount`, `storageUsageDelta`, `suggestedAction`. The log still contains no input/output body — only lengths and metrics.
- Service worker (`public/sw.js`) does not intercept the cross-origin model/runtime fetch (only same-origin static assets + nav + Google Fonts), so it is not the cause. Noted: the SW `activate` cache allow-list would purge a WebLLM model cache on SW version change; left unchanged this phase to avoid SW risk.

Next: retry on PC Chrome (and consider clearing site data); only after a successful PC load, measure iPhone Safari. Measurements are never fabricated.

## Phase 3E Transformers.js Fallback (design + adapter seam)

Phase 3E adds a **fallback engine seam only** — no dependency, no model, nothing bundled — in case the WebGPU/WebLLM path is unavailable or too slow on a device.

Role split:

**WebLLM-style (WebGPU) engine**
- First candidate on WebGPU-`true` devices.
- Needs PC Chrome / iPhone Safari real-device measurement (still incomplete).
- Open issues: first-run model size, generation speed, memory.

**Transformers.js-style (WASM) engine**
- Insurance/fallback candidate for devices where WebGPU is unstable or slow.
- Can potentially run on WASM (no WebGPU required).
- Speed, model quality, and model size are unverified; needs its own spike before any production use.

This phase ships `src/features/embeddedAi/engines/transformersFallbackAdapter.js` as a swappable seam (`loadTransformersFallbackEngine`, `generateWithTransformersFallback`, `registerTransformersFallbackLoader`, `resetTransformersFallbackEngineForTests`). With nothing wired it returns `{ ok:false, reason:"transformers-fallback-not-enabled", message:"Transformers.js fallback is not enabled yet." }`. No real model runs. Not in normal UI; flags stay false. A future engine is wired via `registerTransformersFallbackLoader(loader)` where `loader` dynamic-imports its library inside itself (lazy chunk, never in the initial bundle). Input is clamped to 800 chars, output is plain text (≤3 lines), input/output is processed on-device and never sent to an external AI API, and bodies are never logged.

Status: WebLLM remains on hold and its real-device measurement is still **incomplete**; embedded-AI production UI stays **forbidden** until measured. If WebLLM measures poorly (especially on iOS Safari), this Transformers.js fallback becomes the path to spike next.

## Phase 3D Return to Main-Safe (PoC paused, awaiting measurements)

Phase 3D returns the embedded-AI experiment to a state that is safe to carry on main, while keeping the PoC implementation and docs.

- `EMBEDDED_AI_POC_ENABLED = false`, `EMBEDDED_AI_UI_ENABLED = false`, `EMBEDDED_AI_PROBE_ENABLED = false`. No AI button in normal UI; not in TABS/nav; `App.jsx` not the production entry; legacy boot preserved; Local AI UI still paused; Ollama untouched.
- The WebLLM loader / WebGPU adapter / PoC panel / hidden PoC route stay in the repo but are inert in normal use: reachable only via `?oriexProbe=embedded-ai-poc` / `#embedded-ai-poc`, and even there load/generate require the flag to be true.
- **Dependency `@mlc-ai/web-llm` is KEPT (not reverted)** because build / test / security:scan / audit all pass, it is not in the initial HTML, it is split into a dynamic-import-only lazy chunk, and it is never loaded on a normal URL. The decision is held open pending real-device measurement.
- **Real-device measurement is INCOMPLETE.** GitHub Pages spike-branch deploy was not completed, so iPhone Safari (and PC Chrome) real performance is unverified. **Production UI for embedded AI remains forbidden** until measured.
- If measurements are poor, `@mlc-ai/web-llm` is a **revert candidate**; a **Transformers.js-style small model stays as the fallback** candidate.
- To measure later: on a verification branch set `EMBEDDED_AI_POC_ENABLED = true` temporarily, deploy via `workflow_dispatch` on that branch, measure on device, then **reset to false before anything reaches main** (main must always have the flag false).
