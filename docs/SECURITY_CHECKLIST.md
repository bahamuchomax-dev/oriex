...
- [ ] フェーズ3D main安全状態: `EMBEDDED_AI_POC_ENABLED = false`（UI/Probeも false）。PoC実装・`@mlc-ai/web-llm` 依存は残置だが通常状態で無効。実機測定は未完了で本番UI化は禁止。測定する場合のみ検証ブランチで一時 true 化し、main へ入れる版は必ず false。実測が悪ければ `@mlc-ai/web-llm` は revert 候補、Transformers.js系は保険候補。
- [ ] フェーズ3E Transformers.js fallback: adapter足場のみ（`engines/transformersFallbackAdapter.js`）。実依存・実モデル未追加。未wired時は graceful な not-enabled を返す。導入時も dynamic import 限定で初期bundle非同梱。入力は800字clamp・出力プレーンテキスト・入力/生成本文はログ非記録・外部AI APIへ送信しない。通常UI非表示・フラグ全 false 維持。
