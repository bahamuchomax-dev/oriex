...
## Phase 3 Review Inventory & Hardening

Review (復習) is the next React migration target. **Status: NOT in production.** `main.js` still boots the legacy bundle; `App.jsx` (which references Review and lists a `review` tab) is not the production entry, so the React Review screen is not user-visible. Production review still runs in the legacy app, unchanged.

Current state:
- `src/features/review/Review.jsx` is a prop-driven scaffold (`words`, `history`, `onGrade`, `onBack`). It renders card fields (`card.en` / `card.ja`) via React text interpolation only — no raw-HTML sink, no `innerHTML`. The deck is a random sample with wrong-answered cards first, keyed by `wordKey()`.
- No persistence existed for React Review. Added `src/services/repository/reviewRepository.js` as the safe localStorage seam: `loadHistory` / `recordGrade` / `clearHistory`.

Safety this phase (localStorage only — no Firestore, no Rules change, no auth change):
- Writes are pinned to the current user via `assertOwnUid`; a client-passed uid that differs from the signed-in user is ignored, and with no resolvable owner nothing is saved or deleted.
- An optional free-text `note` is run through `sanitizePlainText` (strips HTML tags + dangerous schemes, drops control chars, trims) and clamped to `REVIEW_NOTE_MAX_LENGTH` (200). Whitespace-only → "".
- History is keyed by `wordKey()`; a card with no resolvable key is rejected without writing.
- All stored values are rendered as plain `{text}`; the screen must never use a raw-HTML sink. Storage failures return `{ ok:false, error }` instead of throwing (no white screen).

Go-live conditions (later): wire Review into the production entry only after the screen, its repository, and the home/Records flows are all on React; when moving to Firestore keep the same load/record/clear surface, scope reads to the current uid behind `readCache`, avoid realtime listeners and unbounded reads, and never read other users' review history.

Embedded AI note: the WebLLM PoC is PAUSED. `EMBEDDED_AI_POC_ENABLED` has been reset to `false` (with `EMBEDDED_AI_UI_ENABLED` / `EMBEDDED_AI_PROBE_ENABLED` also false). The WebLLM dependency/adapter remain in the repo but are fully hidden from normal UI / production navigation.
