...
/* ============================================================
 * Grades — 成績カルテ (LABO: test-score record + trend)
 * ------------------------------------------------------------
 * Renders ONLY its own body inside the existing dark scroll area
 * (.oxv-body, ~390px). The parent already draws the back button +
 * title bar, so no page chrome is rendered here.
 *
 *  - Trend card: a bar sparkline of the recent ~5 tests' 合計点,
 *    the latest score large, and a +/- delta vs the previous test
 *    shown with an up (var(--good)) / down (var(--red)) svg arrow.
 ...
 * try/catch). Every selector in grades.css is prefixed .oxv-gr and
 * the only @keyframes are namespaced oxvGr*. Tokens (--ink/--red/
 * --blue/--gold/--good/--glass/--line/...) cascade from an ancestor.
 * ============================================================ */

const KEY = "oxhLaboGrades";

// shared LABO subject palette
const SUBJECT_COLOR = {
  数学: "#3f8dff",
  英語: "#e8273c",
  国語: "#9a6bff",
...
const IcChart = (
  <svg viewBox="0 0 24 24" aria-hidden="true">
    <path d="M4 4v15a1 1 0 0 0 1 1h15" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M8 15l3.2-4 3 2.4L20 7.5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);
const IcList = (
  <svg viewBox="0 0 24 24" aria-hidden="true">
    <path d="M8 6h12M8 12h12M8 18h12" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
...
// --- delta badge (up = good / down = red / flat = muted) -------------------
function Delta({ value, big }) {
  if (value === null || value === undefined) return null;
  const cls = `oxv-gr-delta${big ? " oxv-gr-delta-lg" : ""}`;
  if (value === 0) {
    return <span className={`${cls} oxv-gr-flat`}>±0</span>;
  }
  const up = value > 0;
  return (
    <span className={`${cls} ${up ? "oxv-gr-up" : "oxv-gr-down"}`}>
      {up ? IcUp : IcDown}
...
export default function Grades() {
  ...
  return (
    <div className="oxv-gr">
      {/* --- trend / summary ------------------------------------------- */}
      <section className="oxv-gr-trend">
        <div className="oxv-gr-trend-head">
          {IcChart}
          <span>成績の推移</span>
          <small>直近 {window5.length} 回</small>
        </div>

        <div className="oxv-gr-trend-top">
          <div className="oxv-gr-latest">
            <span className="oxv-gr-latest-k">最新の合計点</span>
            <span className="oxv-gr-latest-row">
              <b className="oxv-gr-latest-v">{latest ? latest.total : 0}</b>
              <small className="oxv-gr-latest-unit">点</small>
            </span>
            {latest && (
              <span className="oxv-gr-latest-name">
        ...
        <div className="oxv-gr-spark" role="img" aria-label="直近のテスト合計点の推移">
          {window5.map((r, i) => {
            const h = Math.round((0.3 + 0.7 * ((r.total - minT) / span)) * 100);
            const isLast = i === window5.length - 1;
            return (
              <div className="oxv-gr-bar-wrap" key={r.id}>
                <span className="oxv-gr-bar-val">{r.total}</span>
                <div className="oxv-gr-bar-track">
                  <span
                    className={`oxv-gr-bar${isLast ? " oxv-gr-bar-on" : ""}`}
                    style={{ height: `${h}%` }}
                  />
                </div>
                <span className="oxv-gr-bar-x">{md(r.date)}</span>
              </div>
            );
          })}
        </div>
      ...
      <button
        type="button"
        className={`oxv-gr-add${open ? " oxv-gr-add-open" : ""}`}
        onClick={toggleForm}
        aria-expanded={open}
      >
        {open ? IcClose : IcPlus}
      ...
      {open && (
        <form className="oxv-gr-form" onSubmit={submit}>
          <label className="oxv-gr-field">
            <span className="oxv-gr-label">テスト名</span>
            <input
              className="oxv-gr-input"
              type="text"
              value={name}
          ...
          <div className="oxv-gr-grid">
            <label className="oxv-gr-field">
              <span className="oxv-gr-label">点数<i>必須</i></span>
              <input
                className="oxv-gr-input"
                type="number"
                inputMode="numeric"
            ...
            <label className="oxv-gr-field">
              <span className="oxv-gr-label">偏差値<i>任意</i></span>
              <input
                className="oxv-gr-input"
                type="number"
                inputMode="numeric"
          ...
          {touched && !valid && (
            <p className="oxv-gr-err">テスト名と点数（数値）を入力してください。</p>
          )}

          <button type="submit" className="oxv-gr-submit" disabled={!valid}>
            この成績を記録する
          </button>
        </form>
      )}
      ...
      {/* --- history list --------------------------------------------- */}
      <div className="oxv-gr-h">
        {IcList}
        <span>テスト履歴</span>
        <small>{all.length} 件</small>
      </div>

      <div className="oxv-gr-list">
        {all.map((r, i) => (
          <article className="oxv-gr-card" key={r.id}>
            ...
            <div className="oxv-gr-card-main">
              <span className="oxv-gr-total">
                {r.total}<small>点</small>
              </span>
              {r.hensachi !== null && r.hensachi !== undefined && (
                <span className="oxv-gr-hensachi">偏差値 <b>{r.hensachi}</b></span>
              )}
              <span className="oxv-gr-card-delta">
                <Delta value={deltaOf(i)} />
              </span>
            </div>

            {Array.isArray(r.subjects) && r.subjects.length > 0 && (
              <div className="oxv-gr-subs">
                {r.subjects.map((s) => (
                  <span className="oxv-gr-sub" key={s.key}>
                    <i
                      className="oxv-gr-sub-dot"
                      style={{ background: SUBJECT_COLOR[s.key] || SUBJECT_COLOR["その他"] }}
                    />
                    <span className="oxv-gr-sub-k">{s.key}</span>
                    <b className="oxv-gr-sub-v">{s.score}</b>
                  </span>
                ))}
              </div>
            )}
