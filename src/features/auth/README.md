...
- `safeAuthErrorMessage(error)` — maps Firebase Auth errors to safe Japanese UI
  strings; collapses credential-class errors to one message (no account
  enumeration) and never surfaces the raw `error.message`.
