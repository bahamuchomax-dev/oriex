...
import {
  ...
  WEBLLM_MODEL_ID,
  WEBLLM_MODEL_WEIGHTS_SOURCE,
  WEBLLM_RUNTIME_SOURCE,
} from "./engines/webLlmEngineLoader.js";
...
export default function EmbeddedAiPocPanel() {
  ...
  const LOAD_FAIL_GUIDANCE =
    "モデル取得またはキャッシュ保存中に失敗しました。\n" +
    "通信状態を確認して、再読み込み後にもう一度試してください。\n" +
    "ブラウザのサイトデータ削除で改善する場合があります（手動で行ってください）。\n" +
    "入力文や生成結果は保存されていません。";
  ...
  const copyDiagnostic = async () => {
    try {
      if (typeof navigator !== "undefined" && navigator.clipboard && navigator.clipboard.writeText) {
        ...
        setNote("診断ログをコピーしました。");
        return;
      }
    } catch {
      /* fall through to manual copy */
    }
    setNote("コピーできませんでした。下の診断ログを選択してコピーしてください。");
  };
  ...
  const loadModel = async () => {
    ...
    const usageBefore = before.storageEstimate ? before.storageEstimate.usage : null;
    const usageAfter = after.storageEstimate ? after.storageEstimate.usage : null;
    const usageDelta =
      typeof usageBefore === "number" && typeof usageAfter === "number" ? usageAfter - usageBefore : null;

    // Engine/network error string only — never user input or generated text.
    const rawError = res.ok ? "" : (res.error ? String(res.error) : String(res.reason || ""));
    const errorType = res.ok
      ? ""
      : classifyWebLlmLoadError(rawError, {
          hasWebGpu: before.hasWebGpu,
          hasIndexedDb: before.hasIndexedDb,
          secureContext: before.secureContext,
          online: before.online,
        });
    const suggested = res.ok ? "" : suggestedActionForLoadError(errorType);
  ...
  return (
    <div className="rx-home">
      ...
      <div className="rx-talk" style={{ marginTop: 10 }}>
        ...
        <div className="rx-trow"><div className="rx-trow-ls">Model weights: {WEBLLM_MODEL_WEIGHTS_SOURCE}</div></div>
        <div className="rx-trow"><div className="rx-trow-ls">WASM runtime: {WEBLLM_RUNTIME_SOURCE}</div></div>
        <div className="rx-trow">
          <div className="rx-trow-ls">
            これはモデル/ランタイムの取得先です。入力文を外部AI APIへ送信するものではありません。
          </div>
        </div>
      </div>
      ...
      {note && (
        <div className="rx-support-msg" role="alert" style={{ color: "#d4574e", marginTop: 10, whiteSpace: "pre-wrap" }}>{note}</div>
      )}

      {(showRetry || diag) && (
        <div className="rx-rec3" style={{ marginTop: 10 }}>
          {showRetry && (
            <button className="rx-rec-s" onClick={loadModel} disabled={!EMBEDDED_AI_POC_ENABLED || busy}>
              <b>再試行</b>
            </button>
          )}
          {diag && (
            <button className="rx-rec-s" onClick={copyDiagnostic}><b>診断ログをコピー</b></button>
          )}
        </div>
      )}
